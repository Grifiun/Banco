...
public class ControladorCargarArchivo extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        OutputStream ops = new FileOutputStream(nombreArchivo);
        ...
        path = file.getAbsolutePath().replace("/" +file.getName(), "") +"/" +nombreArchivo; 
        //obtenemos la direccion       
        //instanciamos el objeto encargado de analizar el archivo
        
         Leer leer = new Leer(path);//enviamos a analizar el archivo, usando su path o ruta
        try {
            leer.leer();
